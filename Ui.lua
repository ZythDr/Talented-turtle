...
do
	-- Vanilla/Turtle port: dual-spec and pet spec tabs are disabled.
	if true then
		return
	end
	...
	local function MakeTab(parent, type)
		...
		t:SetSize(64, 64)
		t:SetPoint("TOPLEFT", tab, "TOPLEFT", -3, 11)
	...
	local function Tabs_OnEvent(self, event, unit)
	...
	local function MakeTabs(parent)
		...
		baseView.SetTemplate = function(self, template, target)
			Talented.tabs:UpdateCheck(template)
			return prev(self, template, target)
		end
